...
void Myocyte::solve(mreal dt, mreal t0, mreal tF, mreal printRate, mreal saveRate){
    ...
    for (int i = 0; i < tUnits; i++)
        ca_units.push_back(new CaRU(i));

    DataBlock *c_units = (DataBlock*) malloc(ca_units.size() * sizeof(DataBlock));
    SolveStepParameters ssp;

    cudaMalloc((void **) &(ssp.device_c_units), ca_units.size() * sizeof(DataBlock));
    cudaMalloc((void **) &(ssp.device_V), 1 * sizeof(mreal));
    cudaMalloc((void **) &(ssp.device_Nai), 1 * sizeof(mreal));
    cudaMalloc((void **) &(ssp.device_toSave), 1 * sizeof(bool));

    for (int i = 0; i < ca_units.size(); i++)
        c_units[i] = ca_units[i]->toDataBlock();
    
    for (int c = 0; t < tF; c++)
    {
        toSave = (saveRate > 0.0 && c % (int)((1.0 / saveRate)) == 0);
        if (printRate > 0.0 && c % (int)((1.0 / printRate)) == 0)
        ...
        cudaMemcpy(ssp.device_c_units, c_units, ca_units.size() * sizeof(DataBlock), cudaMemcpyHostToDevice);
        cudaMemcpy(ssp.device_V, &c_units[0].V, 1 * sizeof(mreal), cudaMemcpyHostToDevice);
        cudaMemcpy(ssp.device_Nai, &c_units[0].Nai, 1 * sizeof(mreal), cudaMemcpyHostToDevice);
        cudaMemcpy(ssp.device_toSave, &toSave, 1 * sizeof(bool), cudaMemcpyHostToDevice);

        ssp.V = Ith(y,_V_);
        ssp.Nai =  Ith(y,_Nai_);
        ssp.toSave = (toSave) ? 1.0 : 0.0;
        ssp.t = t;
        ssp.dt = dt;
        ssp.saveRate = saveRate;
        ssp.tUnits = tUnits;

        solveCaUnitsStep<<<BLOCKS, THREADS_PER_BLOCK>>>(ssp);

        cudaMemcpy(c_units, ssp.device_c_units, ca_units.size() * sizeof(DataBlock), cudaMemcpyDeviceToHost);
        
        for (int i = 0; i < tUnits; i++)
            ca_units[i]->updateCaruFromDataBlock(&c_units[i]);
    ...
    //         ca_units.at(c)->saveVariables(save_t, outputCaUnitsFilePath);
    //     cout << "Fineshed saving." << endl;
    //     cout << "Saving Currents results." << endl;
    //     for (int c = 0; c < tUnits; c++)
    //         ca_units.at(c)->saveCurrents(save_t, outputCaUnitsFilePath);
    //     cout << "Fineshed saving." << endl;
        // cout << "Saving Extra results." << endl;
        // for (int c = 0; c < tUnits; c++)
        //     ca_units.at(c)->saveExtras(save_t, outputCaUnitsFilePath);
        // cout << "Fineshed saving." << endl;
        // cout<<"Plotting results."<<endl;
        // generatePlotFile();
        // command = "(cd "+outputFilePath+"/;python plot.py)";
    ...
    cudaFree(ssp.device_c_units);
    cudaFree(ssp.device_V);
    cudaFree(ssp.device_Nai);
    cudaFree(ssp.device_toSave);
    free(c_units);
