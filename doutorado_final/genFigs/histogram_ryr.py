...
for s in range(len(_scena_)):    
    ...
    for n in range(len(_nlcc_)):
        data = getORyRCurves(_scena_[s], _nlcc_[n])
    ...
    plt.xlabel(r'$O_{RyR}$ Opening Fraction (%)')
    ...
    plt.grid(ls=':')
    plt.savefig('../exit/'+_scena_[s]+'/hist_ryr.pdf',bbox_inches='tight')
